...
namespace utl
{
  ...
  {   
    ...
    void histc(const Eigen::VectorXd &x, const Eigen::VectorXd &bin_ranges, Eigen::VectorXd &bin_counts)
    {
      ...
      {
        std::cout << "[utl::math::histc] input vector and bin ranges size has to be greater than 0." << std::endl;
        abort();
      }
    ...
    void hist(const Eigen::VectorXd &x, int nbins, Eigen::VectorXd &hist_out, Eigen::VectorXd &bin_centers)
    {
      ...
      if (nbins < 1)
      {
        std::cout << "[utl::math::hist] histogram requires at least one bin." << std::endl;
        abort();
      }
      ...
      // Count
      utl::math::histc(x, bin_ranges, hist_out);
    }
    ...
    Eigen::MatrixXd mvnrnd(const int num_samples, const Eigen::VectorXd &mean, const Eigen::MatrixXd &covar)
    {
      ...
      {
        std::cout << "[utl::math::mvnrnd] covariance matrix must be square." << std::endl;
        abort();
      }
      ...
      {
        std::cout << "[utl::math::mvnrnd] covariance and mean must have same dimensions." << std::endl;
        abort();
      }
      ...
      {
        std::cout << "[utl::math::mvnrnd] covariance matrix must by symmetric." << std::endl;
        abort();
      }
      ...
      {
        std::cout << "[utl::math::mvnrnd] Something went wrong in eigen decomposition." << std::endl;
        abort();
      }
      ...
      {
        std::cout << "[utl::math::mvnrnd] Covariance matrix is not positive semidefinite." << std::endl;
        abort();
      }
