...
namespace utl
{
  namespace stdvec
  {
              ...
              )
    {
      ...
      {
        std::cout << "[utl::stdvec::sort] Unknown sort order. Assuming ascending" << std::endl;
        sort(
          sort_order.begin(), 
          sort_order.end(), 
          index_cmp_asc<std::vector<T>& >(unsorted));    
    ...
    std::vector<TObject> vectorFilter (const std::vector<TObject> &v, const std::vector<bool> &mask)
    {
      ...
      if (v.size() != mask.size())
      {
        std::cout << "[utl::stdvec::vectorFilter] vector and mask must be the same size (" << v.size() << ", " << mask.size() << ")\n";
        return std::vector<TObject>(0);
      }
    ...
    bool writeVectorToFileASCII(const std::vector<Scalar> v, const std::string &filename)
    {
      ...
      if (!file.is_open())
      {
        std::cout << "[utl::stdvec::writeVectorToFileASCII] Could not open file for writing ('" << filename << "')." << std::endl;
        return false;
      }
      else
      {
    ...
    bool readVectorFromFileASCII(std::vector<Scalar> &v, const std::string &filename)
    {
      ...
      if (!file.is_open())
      {
        std::cout << "[utl::stdvec::readVectorToFileASCII] Could not open file for reading ('" << filename << "')." << std::endl;
        return false;
      }
      else
      {
    ...
    /** \brief Compute a occurences of a single value in a vector
    ...
    /** \brief Found indices of vector elements that are equal to a target
     * value.
     *  \param[in]  v a vector of values
     *  \param[in]  target_value value to be searched for
    ...
    void vectorHistogram(const std::vector<Scalar> &v, std::vector<Scalar> &v_unique, std::vector<size_t> &counts, const SortMode sort_mode = UNSORTED)
    {
      ...
      {
        std::vector<size_t> sort_order;
        utl::stdvec::sort<size_t>(counts, counts, sort_order, sort_mode);
